<div class="space-y-4 w-full max-w-4xl mx-auto p-4 bg-content_bg  mb-4 relative">
    ...
    <form method="post" action="/posts" enctype="multipart/form-data" class="-pt-8">
        ...
        <div class="mb-2">
            <input placeholder="What are you thinking about?" name="content" id="postInput" class="w-full bg-content_bg text-white placeholder-placeholder pl-2 rounded-lg focus:outline-none grow">
        </div>

        <!-- Poll Section -->
        <div id="pollSection" class="hidden">
            <div id="pollOptions">
                <input type="text" name="options[]" class="p-2 w-full bg-content_bg border border-divider text-white placeholder-placeholder pl-2 rounded-lg focus:outline-none grow mb-2" placeholder="Yes">
                <input type="text" name="options[]" class="p-2 w-full bg-content_bg border border-divider text-white placeholder-placeholder pl-2 rounded-lg focus:outline-none grow mb-2" placeholder="No">
            </div>
            <button type="button" id="addOptionButton" name="addOptionButton" onclick="addOption()" class="p-2 w-full bg-content_bg border border-divider text-white placeholder-placeholder pl-2 rounded-lg focus:outline-none grow mb-2">Add Another Option</button>
        </div>
        ...
        <div class="flex space-x-2 ml-2">
            ...
            <label for="pollToggle" class="cursor-pointer">
                <input type="checkbox" id="pollToggle" class="focus:outline-none hidden" onchange="togglePollSection()">
...
<script type="text/javascript">
    ...
    function addOption() {
        ...
        const inputCount = optionsDiv.getElementsByTagName('input').length;
        if (inputCount < 6) {
            const input = document.createElement('input');
            input.type = 'text';
            input.name = 'options[]';
            input.classList.add('p-2', 'border', 'border-divider', 'w-full', 'bg-content_bg', 'text-white', 'placeholder-placeholder', 'pl-2', 'rounded-lg', 'focus:outline-none', 'grow', 'mb-2');
    ...
    function togglePollSection() {
        ...
        const pollInputs = pollSection.getElementsByTagName('input');
        if (document.getElementById('pollToggle').checked) {
            pollSection.classList.remove('hidden');
            for (let input of pollInputs) {
                input.required = true;
            }
        } else {
